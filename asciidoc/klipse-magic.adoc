...
(def jjj "{\"description\":\"A spring-summer and vacay-ready top in a cheery (not to mention colorful!) floral print we\'re obsessed with this season. Way-cool outfit idea: Pair it with its matching Frankie shorts. Cotton.Machine wash.Import.Select stores.\",\"g:age_group\":\"kids\",\"g:brand\":\"J.Crew\",\"g:color\":\"Emerald Multi\",\"g:gender\":\"female\",\"g:google_product_category\":\"Apparel & Accessories > Clothing\",\"g:item_group_id\":\"G4327\",\"g:mpn\":[\"1689949373816898\",\"1689949373816746\"],\"g:product_type\":\"Apparel & Accessories > girls_category > shirts > noveltyshirts > girls-spaghetti-strap-top-in-seaside-floral\",\"g:size\":[\"14\",\"8\"],\"id\":[\"99104608715\",\"99104608723\"],\"title\":\"Girls\' spaghetti strap top in seaside floral\",\"url\":\"https://i.s-jcrew.com/is/image/jcrew/G4327_WU9613?$pdp_fs418_3x_zoom$\"}")
...
*Definition*: In computer science, bootstrapping is the process of writing a compiler in the source programming language that it intends to compile. Applying this technique leads to a self-hosting compiler (https://en.wikipedia.org/wiki/Bootstrapping_(compilers)[Wikipedia]).

*Application*: In the context of the Web, once a language is bootstrapped it can run on a web page.
...
[source.clj]
----
(cljsjs/eval-str (cljsjs/empty-state)
               "(map inc [1 2 3])"
               ""
               {:eval cljsjs/js-eval
	       :def-emits-var false}
...
[source.clj]
----
(cljsjs/compile-str (cljsjs/empty-state)
                  "(let [a 1]
                      (+ a 2))"
                  ""
                  {:static-fns false}
...
* *:eval*             - eval function to invoke, see .eval-fn.
* *:load*             - library resolution function, see .load-fn.
* *:static-fns*       - employ static dispatch to specific function arities in emitted JavaScript, as opposed to making use of the `call` construct. Defaults to false. See http://blog.klipse.tech/clojurescript/2016/04/14/static-fns.html[Deep into clojurescript function call mechanisms: static vs. dynamic dispatch]
...
[source.clj]
----
(def compiler-empty-state (cljsjs/empty-state))
(defn eval [exp]
  (cljsjs/eval-str compiler-empty-state
                   exp
                   ""
                   {:eval cljsjs/js-eval
                    :context :expr}
